...
Document.prototype.queryStrict = function <T extends Element>(
    selector: string
): T {
...
Element.prototype.getAttributeStrict = function <T extends string>(
    attribute: string
): T {
...
Element.prototype.getAttributeAsFloatStrict = function (
    attribute: string
): number {
...
Element.prototype.queryStrict = function <T extends Element>(
    selector: string
): T {
    const element = this.querySelector<T>(selector);
    if (!element)
...
URLSearchParams.prototype.getStrict = function <T extends string>(
    name: string
): T {
...
URLSearchParams.prototype.getAsIntegerStrict = function (
    name: string,
    radix: number = 10
): number {
...
// Auxiliares.
function isValidElementSource<T>(
    source: unknown
): source is T[] | NodeList | string {
    if (
...
function parseElementSource<T extends Element[] | NodeList | string>(
    source: T
): Element[] {
